...
render_view (PhocOutput *output, PhocView *view, PhocRenderContext *ctx)
{
  ...
  if (ctx->blur_radius > 0 && phoc_view_wants_blur (view)) {
    struct wlr_box geo;
...
/**
 * phoc_renderer_get_blur_enabled:
 * @self: The renderer
 *
 ...
 *
 * The `blur` gsetting switches the whole effect off on top of that, and so
 * does a renderer that cannot run the effect. Both have to be checked
 * anywhere the blur costs something, not just where it draws: the output
 * damages itself whole when a blurred surface is up, and doing that while
 * this returns %FALSE is all cost and no picture.
 *
 * Returns: %TRUE if a requested blur would actually be rendered
 */
gboolean
phoc_renderer_get_blur_enabled (PhocRenderer *self)
{
...
/*
 * How far down the pyramid a radius reaches.
 *
 * Reach roughly doubles per level, and level 0 already works at a quarter of
 * the output, so one of its texels is four screen pixels. Radius 8 is
 * therefore level 0 alone and each doubling of the radius buys one more level.
 */
static int
blur_levels (guint radius)
{
  int levels = (int) roundf (log2f (MAX (radius, 8u) / 8.0f));

  return CLAMP (levels, 0, PHOC_BLUR_MAX_LEVELS - 1);
}
...
phoc_renderer_capture_blur (PhocRenderer *self, PhocOutput *output, PhocRenderContext *ctx)
{
  ...
  /*
   * Only the bindings we cannot derive are asked for. The viewport is the
   * output's own, since this runs inside the output's render pass, and the
   * active unit is GL_TEXTURE0 -- blur_draw_quad selects it and every texture
   * draw in this renderer sets its own before sampling.
   */
  glGetIntegerv (GL_FRAMEBUFFER_BINDING, &prev_fbo);
  glGetIntegerv (GL_ARRAY_BUFFER_BINDING, &prev_abuf);
  prev_blend = glIsEnabled (GL_BLEND);
   ...
   *
   * The whole of it, deliberately: the pyramid's coarse levels mix content
   * from across the output, so a partial copy would leave the levels reading
   * texels captured at different times. The output damages itself whole while
   * a blur is up, which is what makes this a clean picture of the scene below.
   */
  glBindTexture (GL_TEXTURE_2D, state->capture);
  glCopyTexSubImage2D (GL_TEXTURE_2D, 0, 0, 0, 0, 0, state->width, state->height);
  glBindTexture (GL_TEXTURE_2D, 0);

  levels = blur_levels (radius);
