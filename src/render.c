 ...
 * protocol, so that request is the opt in. The `blur` gsetting switches the
 * whole effect off on top of that.
 */
static gboolean
...
phoc_renderer_render_output (PhocRenderer *self, PhocOutput *output, PhocRenderContext *ctx)
{
  ...
  if (output->fullscreen_view != NULL) {
    ...
    // Render top layer above views; render_layer() captures the blur backdrop
    // itself, just before each surface that asked for one
    render_layer (ZWLR_LAYER_SHELL_V1_LAYER_TOP, ctx);
  }
